...
有 query：返回 === 直接命中记忆 === 和可选的 === 联想浮现 ===；直接命中默认只展示最相关的 1 个 moment，格式含 [bucket_id:...] 和 [moment_id:...]。如果命中结构化正文里的中间片段，会自动带同桶前后语境、affect_anchor / favorite_reason / 年轮等温度片段。扩散也以 moment 为单位进行，bucket 级 memory_edges 会桥接到代表 moment。关键词和 embedding 双通道仍先提供候选 bucket，默认过滤 feel。
...
```text
1. breath(query=...) 现在返回 moment 片段，不再把直接命中整桶脱水。要精确读完整正文/年轮请用 read_bucket(bucket_id)。
