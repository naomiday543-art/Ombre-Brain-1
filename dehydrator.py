...
13. **保留原文的语气和质感**：亲密对话就保留亲密感（"想她想得发疯"、"操"、"操着她"），日常吐槽就保留口语化。**严禁**改写成研究报告腔——比如把"瞳瞳说一天到晚都在做"写成"瞳瞳对亲密互动持高频率偏好"，把"她对量化情绪有意见"写成"她对情感数据化持批判态度，警惕工具理性介入"。原话越赤裸越具体越好，不要文绉绉的概括。
...
8. **保留原文的语气和质感**——亲密、口语、赤裸的表达原样保留，**严禁**合并时改写成研究报告腔（"持批判态度"、"反映了对真实情感连接的珍视"、"成为关系中的推离机制"这类论文式总结一律不要）。原话越具体越好。
